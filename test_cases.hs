...
problem5Tests = [
  ...
  ("(fun ([x : a]) x)", Err "unbound type var(s) in a: [\"a\"]"),
  ("(fun ([x : (num -> a)]) x)", Err "unbound type var(s) in (num -> a): [\"a\"]"),
  ("(fun ([x : (a -> (b, c))]) x)", Err "unbound type var(s) in (a -> (b, c)): [\"a\",\"b\",\"c\"]"),
  ("(forall (a) (fun ([x : a]) x))", Ok (ForAllT "a" (ArrowT (VarT "a") (VarT "a")))),
